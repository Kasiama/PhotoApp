...
extension FriendsViewController : UITableViewDataSource, UITableViewDelegate, UIGestureRecognizerDelegate{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if isAllUsers {
            if isSearching{ return self.sortedAllUsers.count}
            else{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if isSearching{
            if self.isAllUsers{
                       ...
                       if self.sortedAllUsers[indexPath.row].isFriend{
                       cell.statusName.text = "friends"
                       }
                   }
        ...
        if self.isAllUsers{
            let uid = self.allUsers[indexPath.row].id
                   ...
                   cell.friendImageView.loadImageWhithoutUser(idString: "\(uid)/\(uid)")
            if self.allUsers[indexPath.row].isFriend{
            cell.statusName.text = "friends"
            }
        }
