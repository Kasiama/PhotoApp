...
class FullImageViewController: UIViewController {
    ...
    convenience init(id: String, description: String? ){
        self.init()
         Bundle.main.loadNibNamed("FullImageViewController", owner: self, options: nil)
            ...
            let newBackButton = UIBarButtonItem(title: "Back", style: UIBarButtonItem.Style.plain, target: self, action: #selector(backTaped))
    ...
    func load(){
        ...
        let bounds = CGRect.init(x: 0, y: 0, width: UIScreen.main.bounds.width, height: 130)
        ...
        let boundss = CGRect.init(x: 0, y: 570, width: UIScreen.main.bounds.width, height: 130)
